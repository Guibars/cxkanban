...
export default function ExtraCostsView({ costs, currentUser }: ExtraCostsViewProps) {
  ...
  const commercialTotal = costs.filter((cost) => cost.responsible === 'Comercial').reduce((sum, cost) => sum + cost.totalCost, 0);
  ...
  }, [costs, responsibleFilter, search]);

  const insights = useMemo(() => ({
    responsible: aggregateCost(costs, (cost) => cost.responsible),
    regional: aggregateCost(costs, (cost) => cost.regional),
    origin: aggregateCost(costs, (cost) => cost.origin),
    category: aggregateCost(costs, (cost) => cost.reasonCategory),
  }), [costs]);
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Metric label="Ocorrências de custo" value={costs.length.toLocaleString('pt-BR')} icon={Receipt} tone="bg-blue-50 text-blue-700" />
          ...
          {costs.length === 0 ? <div className="rounded-2xl border border-dashed border-gray-300 bg-white/60 p-8 text-center text-sm text-gray-500">Importe a planilha ou cadastre o primeiro custo para visualizar os insights.</div> : (
            <div className="grid gap-4 xl:grid-cols-3">
              ...
              <div className="rounded-2xl border border-white bg-white/85 p-4 shadow-sm">
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 text-center"><CostType label="Produto" value={costs.reduce((sum, cost) => sum + cost.productCost, 0)} /><CostType label="Logística" value={costs.reduce((sum, cost) => sum + cost.logisticsCost, 0)} /><CostType label="Impostos" value={costs.reduce((sum, cost) => sum + cost.taxCost, 0)} /></div>
              </div>
            </div>
          )}
        </section>
      ...
      {costs.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-3xl border border-white/90 bg-white/80 shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4"><div><h2 className="text-sm font-extrabold text-gray-900">Registros de custos extras</h2><p className="text-[11px] text-gray-500">{filtered.length} de {costs.length} registros exibidos</p></div><strong className="text-sm text-[#385041]">{currency(filtered.reduce((sum, cost) => sum + cost.totalCost, 0))}</strong></div>
