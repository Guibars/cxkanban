...
export function exportRaExcel(items: RACase[], period: string) {
  ...
  downloadWorkbook({
    ...
    rows: items.map((item) => ({ date: excelDate(item.createdAt, true), updatedAt: excelDate(item.updatedAt, true), ra: item.raNumber, customer: item.customerName, phone: item.phone, email: item.email, status: item.status, response: item.status === 'Aberto' ? 'Não' : 'Sim', solution: item.status === 'Resolvido' ? 'Sim' : 'Não', score: customerScoreValue(item), return: wouldDoBusinessValue(item) === null ? 'Sem resposta' : wouldDoBusinessValue(item) ? 'Sim' : 'Não', caseScore: item.finalScore ?? null, assignee: item.assigneeName || '', assigneeEmail: item.assigneeEmail || '', createdBy: item.createdByName || '', createdByEmail: item.createdByEmail || '', information: item.information, id: item.id })),
  });
}
